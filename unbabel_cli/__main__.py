from event_parser import parse_from_cli, load_translation_events, filter_events_list, get_boundary_timestamps
from window import sliding_window_loop
from utils import save_results_to_file

def main():

    args = parse_from_cli()
    translation_events = load_translation_events(file_location=args.input_file)
    filtered_events = filter_events_list(events_list=translation_events)
    starting_minute, final_minute = get_boundary_timestamps(events_list=filtered_events)
    averages_list = sliding_window_loop(starting_minute=starting_minute, final_minute=final_minute, window_size= args.window_size, filtered_events=filtered_events)
    save_results_to_file(file_name='output', results_list=averages_list)

if __name__ == "__main__":
    main()


